...
`CoordinatorTransferClient` wraps the Coordinator's authenticated internal prepare, state-advance, commit and abort endpoints. It applies the configured HTTPS endpoint, bearer API key and timeout, and rejects prepare replies for a different transfer ID. The Gateway exposes only the start/reservation step publicly; later transition calls remain service-to-service.
