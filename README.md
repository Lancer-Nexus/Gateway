...
The target-side verification and MySQL/Coordinator target-acceptance commit sequence are available. Source freeze orchestration, authenticated snapshot transport, target snapshot preparation, source release, and GameServer lease enforcement are still pending; the capability does not mean jump-gate travel is end-to-end ready.
